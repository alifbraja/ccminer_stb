...
{
    __m128i *pbuf;
	__m128i pbuf_copy[4] = { _mm_xor_si128(buf[0], buf[2]), _mm_xor_si128(buf[1], buf[3]), buf[2], buf[3] };
	...
	fixrandex[i] = prandex_idx;
        ...
        switch (selector & 0x1c)
        {
            ...
            case 0:
            {
				...
				PREFETCH_T0(randomsource + ((selector >> 5) & keyMask), LIONELK_FETCH_DIST);
				PREFETCH_T0(randomsource + ((selector >> 32) & keyMask), LIONELK_FETCH_DIST);
            ...
            case 0x18:
            {
				__m128i *buftmp = pbuf - (((selector & 1) << 1) - 1);
				__m128i tmp; // used by MIX2
